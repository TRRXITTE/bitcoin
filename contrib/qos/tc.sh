...
#limit outgoing traffic to and from port 8334. but not when dealing with a host on the local network
...
if [ -n "${LOCALNET_V6}" ] ; then
	ip6tables -t mangle -A OUTPUT -p tcp -m tcp --dport 8334 ! -d ${LOCALNET_V6} -j MARK --set-mark 0x4
	ip6tables -t mangle -A OUTPUT -p tcp -m tcp --sport 8334 ! -d ${LOCALNET_V6} -j MARK --set-mark 0x4
fi
