...
public:
    UniValue PrepareRequest(const std::string& method, const std::vector<std::string>& args) override
    {
        ...
        nblocks = ToIntegral<int>(args.at(0)).value_or(0);
    ...
    UniValue ProcessReply(const UniValue& reply) override
    {
        // If nblocks is negative, we'll handle the infinite loop in CommandLineRPC instead
        UniValue result(UniValue::VOBJ);
...
protected:
    ...
    int nblocks{0}; // Store nblocks to check for indefinite generation
};
...
static int CommandLineRPC(int argc, char *argv[])
{
    ...
    try {
        ...
        if (nRet == 0) {
            ...
            if (gArgs.GetBoolArg("-generate", false) && dynamic_cast<GenerateToAddressRequestHandler*>(rh.get())) {
                GenerateToAddressRequestHandler* gta_rh = static_cast<GenerateToAddressRequestHandler*>(rh.get());
                int nblocks = ToIntegral<int>(args[0]).value_or(0);
                if (nblocks < 0) {
                    // Indefinite generation mode
                    UniValue result(UniValue::VARR);
                    int batch_size = 1; // Generate 1 block at a time for responsiveness
                    tfm::format(std::cout, "Generating blocks indefinitely to address %s (Ctrl+C to stop)...\n", args[1].c_str());
                    
                    while (true) {
                        try {
                            args[0] = ToString(batch_size); // Update nblocks in args for each call
                            UniValue reply = ConnectAndCallRPC(gta_rh, method, args, wallet_name);
                            const UniValue& blocks = reply.find_value("result")["blocks"];
                            for (const UniValue& hash : blocks.getValues()) {
                                result.push_back(hash);
                                tfm::format(std::cout, "Generated block: %s\n", hash.get_str().c_str());
                            }
                            std::this_thread::sleep_for(std::chrono::milliseconds(100)); // Brief pause for interruption
                        } catch (const std::exception& e) {
                            tfm::format(std::cerr, "Error: %s\n", e.what());
                            nRet = EXIT_FAILURE;
                            break;
                    ...
                    if (error.isNull()) {
                        ParseResult(reply.find_value("result"), strPrint);
                    } else {
                        ParseError(error, strPrint, nRet);
                    }
                }
                ...
                // Other RPC calls
                UniValue reply = ConnectAndCallRPC(rh.get(), method, args, wallet_name);
