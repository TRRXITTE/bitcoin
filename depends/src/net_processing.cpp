...
bool PeerManagerImpl::TipMayBeStale()
{
    ...
    return m_last_tip_update.load() < GetTime<std::chrono::seconds>() - consensusParams.PowTargetSpacing(tipHeight) * 3 && mapBlocksInFlight.empty();
}

int64_t PeerManagerImpl::ApproximateBestBlockDepth() const
{
    LOCK(cs_main); // Add lock to satisfy thread safety
    int bestHeight = m_chainman.ActiveChain().Height(); // Safe now with cs_main held
    int64_t targetSpacing = m_chainparams.GetConsensus().GetPowTargetSpacing(bestHeight);
    return (GetTime<std::chrono::seconds>() - m_best_block_time.load()).count() / targetSpacing;
}

bool PeerManagerImpl::CanDirectFetch()
{
    int tipHeight = m_chainman.ActiveChain().Height();
    return m_chainman.ActiveChain().Tip()->Time() > NodeClock::now() - m_chainparams.GetConsensus().PowTargetSpacing(tipHeight) * 20;
}
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::GETBLOCKS) {
        ...
        for (; pindex; pindex = m_chainman.ActiveChain().Next(pindex))
        {
            ...
            const int tipHeight = m_chainman.ActiveChain().Height();
            const int nPrunedBlocksLikelyToHave = MIN_BLOCKS_TO_KEEP - 3600 / m_chainparams.GetConsensus().GetPowTargetSpacing(tipHeight);
...
bool PeerManagerImpl::SendMessages(CNode* pto)
{
...

    {
                ...
                if (MaybeSendGetHeaders(*pto, GetLocator(pindexStart), *peer)) {
                    ...
                    peer->m_headers_sync_timeout = current_time + HEADERS_DOWNLOAD_TIMEOUT_BASE +
                        (
                         ...
                         Ticks<std::chrono::seconds>(NodeClock::now() - m_chainman.m_best_header->Time()) / consensusParams.GetPowTargetSpacing(m_chainman.m_best_header->nHeight)
                        );
                    nSyncStarted++;
                }
            }
        ...
        if (state.vBlocksInFlight.size() > 0) {
            ...
            int heightInFlight = state.vBlocksInFlight.front().pindex->nHeight; // Height of the first block in flight
            int64_t targetSpacing = consensusParams.GetPowTargetSpacing(heightInFlight);
        if (current_time > state.m_downloading_since + std::chrono::seconds{targetSpacing} * (BLOCK_DOWNLOAD_TIMEOUT_BASE + BLOCK_DOWNLOAD_TIMEOUT_PER_PEER * nOtherPeersWithValidatedDownloads)) {
