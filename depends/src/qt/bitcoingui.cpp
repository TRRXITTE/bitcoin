...
void BitcoinGUI::updateHeadersSyncProgressLabel()
{
    ...
    int estHeadersLeft = (GetTime() - headersTipTime) / Params().GetConsensus().PowTargetSpacing(clientModel->getNumBlocks()).count();
...
{
    int estHeadersLeft = blockDate.secsTo(QDateTime::currentDateTime()) / Params().GetConsensus().PowTargetSpacing(clientModel->getNumBlocks()).count();
    if (estHeadersLeft > HEADER_HEIGHT_DELTA_SYNC)
