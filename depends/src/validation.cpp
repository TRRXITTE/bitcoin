...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams) {
    ...
    else if (nHeight <= 653142) {
        return nSubsidy / 2;  // 25 TRRXITTE
    }
    else if (nHeight <= 1216284) {
        return 20 * COIN;
    }
    else if (nHeight <= 1779426) {
        return 15 * COIN;
    }
    else if (nHeight <= 2342568) {
        return 10 * COIN;
    }
    else if (nHeight <= 3479427) {
        return 5 * COIN;
    }
    else if (nHeight <= 4042569) {
        return 4 * COIN;
    }
    else if (nHeight <= 4605711) {
        return 3 * COIN;
    }
    else if (nHeight <= 5168853) {
        return 2 * COIN;
    }
    else if (nHeight <= 6305711) {
        return 1 * COIN;
    }
    else {
        CAmount totalSupply = 
            (100000LL * 50 * COIN) +
            ((653142LL - 100000) * 25 * COIN) +
            ((1216284LL - 653142) * 20 * COIN) +
            ((1779426LL - 1216284) * 15 * COIN) +
            ((2342568LL - 1779426) * 10 * COIN) +
            ((3479427LL - 2342568) * 5 * COIN) +
            ((4042569LL - 3479427) * 4 * COIN) +
            ((4605711LL - 4042569) * 3 * COIN) +
            ((5168853LL - 4605711) * 2 * COIN) +
            ((6305711LL - 5168853) * 1 * COIN);

        CAmount targetSupply = 100000000LL * COIN;
        CAmount remainingSupply = targetSupply - totalSupply;
        int64_t blocksRemaining = (remainingSupply + (5 * COIN / 2) - 1) / (5 * COIN / 2);
        int64_t cutoffHeight = 6305712 + blocksRemaining;  // ~23,921,711

        if (nHeight < cutoffHeight) {
            return (5 * COIN) / 2;  // 2.5 TRRXITTE
        }
    }
...
bool ChainstateManager::ProcessNewBlockHeaders(std::span<const CBlockHeader> headers, bool min_pow_checked, BlockValidationState& state, const CBlockIndex** ppindex)
{
    ...
    if (NotifyHeaderTip()) {
        if (IsInitialBlockDownload() && ppindex && *ppindex) {
            ...
            int64_t blocks_left{std::chrono::duration_cast<std::chrono::seconds>(NodeClock::now() - last_accepted.Time()).count() / GetConsensus().PowTargetSpacing(last_accepted.nHeight).count()};
...
void ChainstateManager::ReportHeadersPresync(const arith_uint256& work, int64_t height, int64_t timestamp)
{
    ...
    if (initial_download) {
        int64_t blocks_left{std::chrono::duration_cast<std::chrono::seconds>(NodeClock::now() - NodeSeconds{std::chrono::seconds{timestamp}}).count() / GetConsensus().PowTargetSpacing(height).count()};
        blocks_left = std::max<int64_t>(0, blocks_left);
