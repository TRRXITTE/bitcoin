traaittXT Enterprise version 0.19.0.1 is now available from:
...
traaittXT Enterprise is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use traaittXT Enterprise on unsupported systems.
...
to open the traaittXT Enterprise .dmg. This is due to new signing requirements
imposed by Apple, which the traaittXT Enterprise project does not yet adhere too.
...
- [Reduce memory](https://github.com/bitcoin/bitcoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running traaittXT Enterprise on systems with
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile traaittXT Enterprise with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in traaittXT Enterprise 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of traaittXT Enterprise. (#14054)

- To eliminate well-known denial-of-service vectors in traaittXT Enterprise,
  ...
  This prevents traaittXT Enterprise from sending the BIP111 NODE_BLOOM service
  ...
  of traaittXT Enterprise and nodes that have manually enabled BIP37 support,
...
- By default, traaittXT Enterprise will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how traaittXT Enterprise determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  traaittXT Enterprise would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, traaittXT Enterprise will be bound to
  the standard port 8100 even if a different port is configured for
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in traaittXT Enterprise v0.17.0), this can eliminate a
...
- #15896 feature_filelock, interface_bitcoin_cli: Use PACKAGE_NAME in messages rather than hardcoding traaittXT Enterprise (luke-jr)
