...
traaittXT Enterprise version 0.21.0 is now available from:
...
From traaittXT Enterprise 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, traaittXT Enterprise does not yet change appearance
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  Version 2 hidden services are still fully supported by traaittXT Enterprise, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. traaittXT Enterprise will no longer
...
- The `startupnotify` option is used to specify a command to
  execute when traaittXT Enterprise has finished with its startup
  sequence. (#15367)
...
traaittXT Enterprise will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`bitcoin.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
traaittXT Enterprise will now just log warnings instead of creating new wallets with
...
Users who already have a traaittXT Enterprise wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of traaittXT Enterprise.
...
The change to using descriptors changes the default derivation paths used by traaittXT Enterprise
...
- A new `fee_rate` parameter/option denominated in satoshis per vbyte (sat/vB)
  ...
  `walletcreatefundedpsbt` still exists for setting a fee rate in XTEcash per 1,000
  vbytes (XTEcash/kvB), but it is expected to be deprecated soon to avoid
  confusion. For these RPCs, the fee rate error message is updated from XTEcash/kB
  to sat/vB and the help documentation in XTEcash/kB is updated to XTEcash/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from XTEcash/kvB to sat/vB and the
  ...
  difference between XTEcash/kvB and sat/vB units means that a transaction with a
  fee rate mistakenly calculated in XTEcash/kvB rather than sat/vB should raise an
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time traaittXT Enterprise is
...
- #18739 Document how to fuzz traaittXT Enterprise using Honggfuzz (practicalswift)
