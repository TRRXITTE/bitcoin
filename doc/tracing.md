# User-space, Statically Defined Tracing (USDT) for traaittXT Enterprise

traaittXT Enterprise includes statically defined tracepoints to allow for more
...
**Note**: traaittXT Enterprise uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to traaittXT Enterprise
...
To list probes in traaittXT Enterprise, use `info probes` in `gdb`:
...
The `readelf` tool can be used to display the USDT tracepoints in traaittXT Enterprise.
